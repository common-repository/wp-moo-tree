...
var MooTreeControl = new Class({
	...
	initialize: function(config, options) {
		...
		this.theme = config.theme || moo_file.thefile + 'mootree.gif';
		
		this.loader = config.loader || {icon: moo_file.thefile + 'mootree_loader.gif', text:'Loading...', color:'#a0a0a0'};
...
var MooTreeNode = new Class({
	...
	initialize: function(options) {
		...
		this.classi = options.classi || '';
		this.linkcaption = options.caption || '';
		this.linkdesc = options.description || '';
		...
		this.div = {
			main: new Element('div').addClass('mooTree_node').set('id', this.id),
